...
class RouterBenchmark {
    ...
    fun setup() {
        ...
        router.addRoute(HttpMethod.GET, "/api/users") { _ ->
            Response(status = HttpStatus.OK, body = Body.Strict("users".toByteArray()))
        }

        router.addRoute(HttpMethod.GET, "/api/users/:id") { req ->
            val id = req.pathParams.string("id")
            Response(status = HttpStatus.OK, body = Body.Strict("user $id".toByteArray()))
        }
        ...
        router.addRoute(HttpMethod.GET, "/api/users/:userId/posts/:postId") { req ->
            ...
            Response(status = HttpStatus.OK, body = Body.Strict("user $userId post $postId".toByteArray()))
        }

        router.addRoute(HttpMethod.POST, "/api/users") { _ ->
            Response(status = HttpStatus.CREATED, body = Body.Strict("created".toByteArray()))
        }

        router.addRoute(HttpMethod.PUT, "/api/users/:id") { req ->
            val id = req.pathParams.string("id")
            Response(status = HttpStatus.OK, body = Body.Strict("updated $id".toByteArray()))
        }
