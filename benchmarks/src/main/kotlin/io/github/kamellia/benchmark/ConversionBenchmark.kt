...
class ConversionBenchmark {
    ...
    fun setup() {
        ...
        simpleResponse = Response(
            ...
            body = Body.Strict("ok".toByteArray()),
        )
        ...
        responseWithBody = Response(
            ...
            body = Body.Strict("""{"data":[1,2,3,4,5],"total":5,"status":"success"}""".toByteArray()),
        )
    }
    ...
    fun benchmarkSimpleResponseConversion(blackhole: Blackhole) {
        val nettyResponse = ResponseConverter.convertFull(simpleResponse)
    ...
    fun benchmarkResponseConversionWithHeaders(blackhole: Blackhole) {
        val nettyResponse = ResponseConverter.convertFull(responseWithHeaders)
        blackhole.consume(nettyResponse)
        nettyResponse.release()
    }
    ...
    fun benchmarkResponseConversionWithBody(blackhole: Blackhole) {
        val nettyResponse = ResponseConverter.convertFull(responseWithBody)
        blackhole.consume(nettyResponse)
        nettyResponse.release()
    }
}
